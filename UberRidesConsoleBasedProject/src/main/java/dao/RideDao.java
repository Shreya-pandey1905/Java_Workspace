...
public class RideDao {
    ...
    //for a single customer(req by customer)
    ...
    public boolean cancelRide(long rideId,long customerId) throws SQLException {
        String sql = "update rides SET status= 'CANCELLED' where id=? and customer_id=? and status='REQUESTED'";
        try (Connection connection = DBConnection.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
...
//accepted by driver
    ...
    public boolean completeRide(long rideId,long driverId) throws SQLException {
        String sql = "update rides SET status='COMPLETED' where id=? and driver_id=? and status='ACCEPTED'";
        try (Connection connection = DBConnection.getConnection();
             PreparedStatement statement = connection.prepareStatement(sql)) {
            statement.setLong(1, rideId);
